...
static inline void cluPrintProgramBuildLog(cl_program program, cl_device_id device) {
	...
	if(err != CL_SUCCESS || log_size <= 1) { return; }
	...
	if(err == CL_SUCCESS && log_size > 1) {
		size_t last = log_size - 1;
		while(last > 0 && (log[last] == '\0' || log[last] == '\n' || log[last] == ' ' || log[last] == '\r')) {
			last--;
		}
		if(last > 0) { fprintf(stderr, "OpenCL build log:\n%s\n", log); }
	} else {

...
FILES = [
    ...
    "results_ifi_rtx.csv",
    "results_ifi_amd.csv",
    "results_ifi_amd_optimised.csv",
    "results_ifi_amd_optimised_v2.csv",  # neu: weiter optimierte AMD-Version
]
...
DEVICE_INFO = {
    ...
    "ifi_amd": "ifi – AMD GPU",
    "ifi_amd_optimised": "ifi – AMD GPU (optimised)",
    "ifi_amd_optimised_v2": "ifi – AMD GPU (optimised v2)",  # neu
}
...
for f in FILES:
    ...
    if "precision" in df_part.columns:
        df_part["precision"] = df_part["precision"].astype(str).str.strip()
...
# Aggregierte Daten für Mittelwert + Std
...
# -------------------------------------------------------
# Plot 1: Runtime vs N per device – Mean + Std-Errorbars
# -------------------------------------------------------

def plot_runtime_vs_N_per_device(log_y: bool = False) -> None:
    """
    Für jedes Device:
      x: N (als Kategorien gleich weit auseinander)
      y: elapsed_ms_mean
      Darstellung: Mittelwert + Fehlerbalken (Standardabweichung) pro Precision
    """
    devices = sorted(df_mean["device"].unique())

    for dev in devices:
        dev_mean_dev = df_mean[df_mean["device"] == dev]
        if dev_mean_dev.empty:
            continue

        # Alle N-Werte für dieses Device, sortiert
        Ns_all = sorted(dev_mean_dev["N"].unique())
        ...
        for prec, marker in zip(["float", "double"], ["o", "s"]):
            prec_mean = (
                dev_mean_dev[dev_mean_dev["precision"] == prec]
                .sort_values("N")
            )
            if prec_mean.empty:
                continue

            xs = prec_mean["N"].map(index_of).values.astype(float)
            means = prec_mean["elapsed_ms_mean"].values.astype(float)
            stds = prec_mean["elapsed_ms_std"].values.astype(float)

            # Fehlerbalken: Mittelwert ± Std
            plt.errorbar(
                xs,
                means,
                yerr=stds,
                fmt=marker,        # Marker-Stil
                linestyle="None",  # keine Verbundlinie
                markersize=8,
                capsize=5,         # kleine "Häkchen" am Ende der Balken
                label=prec,
            )
        ...
        plt.title(f"{dev_name} – Runtime vs N (Mean ± Std){scale_label_y}")
        ...
        fname = f"runtime_vs_N_errorbars_{dev}"
        if log_y:
            fname += "_logy"
        fname += ".png"
...
# -------------------------------------------------------
# Plot 2: Device comparison per precision – Mean + Std-Errorbars
# -------------------------------------------------------
...
def plot_device_comparison_per_precision(log_y: bool = False) -> None:
    ...
    Für jede Precision (float / double):
      ...
      y: elapsed_ms_mean
      Darstellung: Mittelwert + Fehlerbalken für jedes Device
    """
    for prec in ["float", "double"]:
        data_prec_mean = df_mean[df_mean["precision"] == prec]
        if data_prec_mean.empty:
        ...
        Ns_all = sorted(data_prec_mean["N"].unique())
        index_of = {n: i for i, n in enumerate(Ns_all)}

        devices = sorted(data_prec_mean["device"].unique())
        markers = ["o", "s", "D", "^", "v", "P", "X"]  # genug Marker für mehrere Devices

        for dev, marker in zip(devices, markers):
            dev_mean = (
                data_prec_mean[data_prec_mean["device"] == dev]
                .sort_values("N")
            )
            if dev_mean.empty:
                continue

            xs = dev_mean["N"].map(index_of).values.astype(float)
            means = dev_mean["elapsed_ms_mean"].values.astype(float)
            stds = dev_mean["elapsed_ms_std"].values.astype(float)
        ...
        plt.ylabel("Time (ms)")
        scale_label_y = " (log y-scale)" if log_y else ""
        plt.title(f"Device comparison – {prec} (Mean ± Std){scale_label_y}")
        ...
        fname = f"device_comparison_errorbars_{prec}"
        if log_y:
            fname += "_logy"
        fname += ".png"
...
def plot_bar_largest_N() -> None:
    ...
    for prec in ["float", "double"]:
        ...
        for dev in devices:
            ...
            labels.append(DEVICE_INFO.get(dev, dev))

...
__kernel void jacobi_step_double_local(const __global double* u, __global double* tmp, const __global double* f,
	...
	if(get_local_id(0) == 0 && i > 0) { tile[local_idx - 1] = u[idx - 1]; }
	if(get_local_id(0) == get_local_size(0) - 1 && i < N - 1) { tile[local_idx + 1] = u[idx + 1]; }
	if(get_local_id(1) == 0 && j > 0) { tile[local_idx - pitch] = u[idx - N]; }
	if(get_local_id(1) == get_local_size(1) - 1 && j < N - 1) { tile[local_idx + pitch] = u[idx + N]; }
